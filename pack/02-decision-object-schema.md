# sw-buffet page 02: Decision Object Schema
...
decision_id:
title:
phase:            # P0-P11 or P2.5 (START-HERE decision phases); page 13 C-steps are recorded under their P phase
owner_agent:      # the recommending role; never the approver of hard-gate exceptions or accepted risk
...
hard_gates:
  ...
  sla_slo:
  vendor_lockin:
  irreversible_cost_escalation:
  unverified_critical_dependency:
  financial_runaway:     # page 13
  cost_abuse:            # page 13
  billing_integrity:     # page 13
  agent_runtime:         # P11 hard-gate list: over-privileged tools or skills, untrusted MCP/tool sources, unrestricted network egress with private data access, secrets exposed to prompts/logs/traces, unreviewed autonomous side effects, prompt-injection exposure, unsafe persistent memory, unsupported claims about the target agent surface
  automated_abuse:       # anti-scraping hard gate: scraping, unrestricted resource consumption, sensitive or paid-content harvesting, credentialed bulk extraction, crawler-policy ambiguity, bot-management false positives
selected_option:
approved_by:             # named human; required for hard-gate exceptions, accepted risks, and spend ceilings. An approval recorded in an earlier engagement counts only after the human confirms it (START-HERE human approval rule)
verification_plan:
metrics:
review_triggers:
linked_artifacts:
...
Status and gate results
Decision status is one of proposed | accepted | rejected | deferred | needs-evidence. Implementation gate results (pass | needs-evidence | blocker, page 12) are not decision statuses: a blocker leaves the affected decision rejected or needs-evidence, never accepted; a needs-evidence result keeps it needs-evidence; a pass changes nothing.

Hard-gate rule
Weighted score cannot override a hard gate. A decision with any unresolved hard gate (START-HERE merged list) stays needs-evidence or rejected, unless the human approves an exception (approved_by); it may then be accepted with the exception recorded.
...
```
Hard-gate slots for financial runaway, cost abuse, and billing integrity are part of the single hard_gates block in the decision object template above.
