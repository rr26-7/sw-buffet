...
Single source of truth stays in pack/ and core/ - this script only renders
them (evals/ is not published: it holds answer keys). GitHub blocks crawlers from /*/tree/ URLs (see
...
# The site renders one release; pages.yml sets SW_BUFFET_VERSION to the latest tag.
VERSION = os.environ.get("SW_BUFFET_VERSION") or "main"
BLOB = f"{REPO_URL}/blob/{VERSION}"
...
def sources() -> list[tuple[Path, str, str]]:
    ...
    # evals/ is not published: its Expected sections and grader notes are answer
    # keys, and agents under evaluation must not be able to read them.
    return items


def link_pack_refs(body: str, pages: dict[str, str]) -> str:
    """Turn in-text 'page NN', 'START-HERE', and 'deployment-contract' into links."""
    by_ref = {}
    for src, out in pages.items():
        name = src.rsplit("/", 1)[-1]
        if src.startswith("pack/"):
            if name[:2].isdigit():
                by_ref[f"page {name[:2]}"] = out
            elif name == "START-HERE.md":
                by_ref["START-HERE"] = out
            elif name == "deployment-contract.md":
                by_ref["deployment-contract"] = out

    def repl(m: re.Match[str]) -> str:
        out = by_ref.get(m.group(1))
        return f'<a href="{SITE_ROOT}/{out}">{m.group(0)}</a>' if out else m.group(0)

    head, sep, rest = body.partition("</h1>")
    if not sep:
        head, rest = "", body
    # only in text outside tags and outside existing links or code
    parts = re.split(r"(<a\b.*?</a>|<code>.*?</code>|<pre>.*?</pre>|<[^>]+>)", rest, flags=re.S)
    pat = re.compile(r"(?<![\w/.-])(page \d{2}|START-HERE|deployment-contract)(?![\w-]|\.\w)")
    parts = [p if p.startswith("<") else pat.sub(repl, p) for p in parts]
...
def nav(items: list[tuple[Path, str, str]], current: str) -> str:
    ...
    for md, rel, grp in items:
        ...
        label = html.escape(title.replace("sw-buffet ", "", 1))
...
{nav}
<main>
<p class="src">Release {version} &middot; <a href="{source}">Source on GitHub</a> &middot; <a href="{repo}">rr26-7/sw-buffet</a><br>For reading only: agents load the pack from the pinned raw files in the README usage prompt, not from this site.</p>
{body}
</main>
...
def main() -> int:
    ...
    for md_path, rel, _ in items:
        ...
        title = INDEX_TITLE if rel == "index.html" else \
            first_heading(text, md_path.stem)
        canonical = f"{BASE_URL}/" if rel == "index.html" else f"{BASE_URL}/{rel}"
        # Pack pages put one statement per line; render each line as its own line.
        ext = MD_EXT + ["nl2br"] if src_rel.parts[0] == "pack" else MD_EXT
        body = markdown.markdown(text, extensions=ext)
        body = rewrite_links(body, src_rel, pages)
        body = link_pack_refs(body, pages)
        if rel == "index.html":
    ...
    if static.is_dir():
        for f in static.rglob("*"):
            # static/README.md documents the folder; it is not site content.
            if f.is_file() and f.relative_to(static).as_posix() != "README.md":
...
def contents_block(items: list[tuple[Path, str, str]]) -> str:
    ...
    for md, rel, grp in items:
        ...
        title = html.escape(first_heading(text, md.stem).replace("sw-buffet ", "", 1))
        desc = html.escape(description(text))
