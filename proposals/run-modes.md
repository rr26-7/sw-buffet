...
Implemented in: pack/START-HERE.md, core/MINIMAL-CORE.md, page 00, deployment-contract
validation, checks (check_run_modes, check_source_of_truth), eval-10.
Deferred: an unnumbered eval for the finished-app-after-one-pass claim (eval-11
is now the comment-decay eval) — add if eval runs show the multi-pass rule is not
holding.
Note: the checks were implemented as check_run_modes markers ("Run modes and
multi-pass execution", "Audit mode: read-only", "default to audit mode"), not
the "Development mode" marker proposed in section 4.
...
>   declared only by the Decision Steward checklist.
...
- (optional, unnumbered): agent claims a finished application after one pass. Fail.
